# AT90 Microcontroller Lab Reaction Game
...
ISR(USART1_RX_vect) {     char received_char = UDR1;     // Echo the character back to the terminal     while (!(UCSR1A & (1 << UDRE1)));     UDR1 = received_char;
...
*/ void timer1_init(void) {     TCCR1A = 0;
...
void game_over(uint16_t rt) {     PORTB |= (1 << PB6);     uint8_t ledn = get_active_led_number();     char msg[80];
...
*/ int main(void) {     srand(0);
    ...
    while (1) {
        ...
        while (keep_playing) {
            ...
            if (timeout_occurred) {                 game_over(reaction_time);                 keep_playing = false;
                ...
                if (user == correct && reaction_time <= time_limit) {                     char ok[80];
